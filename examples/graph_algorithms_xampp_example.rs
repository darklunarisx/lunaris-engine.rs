...
use lunaris_engine::graph::kruskal::kruskal;
use lunaris_engine::graph::prim::prim;
use lunaris_engine::graph::kosaraju_scc::kosaraju_scc;
use lunaris_engine::graph::articulation_points::articulation_points;
...
fn main() {
    ...
    graph.insert(3, vec![]);
    graph.insert(4, vec![]);
    let dfs_order = lunaris_engine::graph::dfs::dfs(&graph, 1);
    println!("DFS order: {:?}", dfs_order);

    // BFS
    let bfs_order = lunaris_engine::graph::bfs::bfs(&graph, 1);
    ...
    wgraph.insert(4, vec![]);
    let dist = lunaris_engine::graph::dijkstra::dijkstra(&wgraph, 1);
    ...
    let bf_dist = lunaris_engine::graph::bellman_ford::bellman_ford(&bf_graph, 1);
    ...
    let fw_dist = lunaris_engine::graph::floyd_warshall::floyd_warshall(&nodes_fw, &edges_fw);
    ...
    dag.insert(0, vec![]);
    dag.insert(1, vec![]);
    let topo = lunaris_engine::graph::topological_sort::topological_sort(&dag);
    println!("Topological sort: {:?}", topo);

    // Union-Find
    let mut uf = lunaris_engine::graph::union_find::UnionFind::new();
    ...
    let mut comps = lunaris_engine::graph::connected_components::connected_components(&cc_graph);
    ...
    println!("Has cycle: {}", lunaris_engine::graph::cycle_detection::has_cycle(&cycle_graph));
    cycle_graph.insert(3, vec![]);
    println!("Has cycle (after removal): {}", lunaris_engine::graph::cycle_detection::has_cycle(&cycle_graph));
    ...
    println!("Is bipartite: {}", lunaris_engine::graph::bipartite_graph::is_bipartite(&bipartite_graph));
    bipartite_graph.insert(4, vec![2, 3, 1]);
    println!("Is bipartite (after making non-bipartite): {}", lunaris_engine::graph::bipartite_graph::is_bipartite(&bipartite_graph));
}
