...
//! # Example
//! ```rust
//! use lunaris_engine::matrix::flood_fill::*;
...
pub fn flood_fill<T: Clone + PartialEq>(grid: &mut Vec<Vec<T>>, sr: usize, sc: usize, new_val: T) {
    ...
    fn dfs<T: Clone + PartialEq>(
        ...
        if r >= rows || c >= cols || grid[r][c] != *orig {
            return;
        }
        grid[r][c] = new_val.clone();
        if r > 0 {
            dfs(grid, r - 1, c, orig, new_val, rows, cols);
        }
        if c > 0 {
            dfs(grid, r, c - 1, orig, new_val, rows, cols);
        }
        if r + 1 < rows {
            dfs(grid, r + 1, c, orig, new_val, rows, cols);
        }
